...
void NextStage(void)
{
	if (g_fail_count == 5)
	{
		...
		GotoXY(28, 12); printf("└────────────────────────┘");
		GotoXY(23, 14); printf("아무키나 두번누르면 게임이 종료됩니다.");
		system("pause > null");
		exit(0);
	}
...
void StageScreen(void)
{
	...
	if (stage < 5)
	{
		...
		GotoXY(28, 13); printf("└─────────────────────────┘");
		GotoXY(22, 15); printf("다음 STAGE를 시작하려면 아무키나 눌러주세요.");
		system("pause > null");
		stage += 1;
		InitScreen();
		Run();
...
void pause(void)
{
	GotoXY(30, 10); printf("잠시 게임을 멈추셨습니다");
	GotoXY(21, 12); printf("게임을 계속하시려면 아무키나 눌러주세요");
	_getch();
	system("cls");
	InitScreen();
}